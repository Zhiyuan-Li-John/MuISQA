...
const defaultFeConfigs: FastGPTFeConfigsType = {
  ...
  systemTitle: '知识库',
  concatMd: '',
  register_method: ['local'], // 恢复注册功能，支持邮箱注册
  limit: {
...
async function getDefaultTeamMemberInfo() {
  try {
    // 从 team_members 集合中获取第一个团队成员信息
    const firstTeamMember = await MongoTeamMember.findOne({}).lean();

    if (firstTeamMember) {
      console.log('load default team member info', firstTeamMember);
      return {
        defaultTeamId: String(firstTeamMember.teamId),
        defaultTmbId: String(firstTeamMember._id),
        defaultUserId: String(firstTeamMember.userId)
      };
    }
    ...
    console.error('Error fetching default team member info:', error);
    return {
      defaultTeamId: '',
      defaultTmbId: '',
      defaultUserId: ''
...
export async function initSystemConfig() {
  // load config
  const [{ fastgptConfig, licenseData }, fileConfig, defaultTeamMemberInfo] = await Promise.all([
    getFastGPTConfigFromDB(),
    readConfigData('config.json'),
    getDefaultTeamMemberInfo()
  ]);
  ...
  const config: FastGPTConfigFileType = {
    ...
    systemEnv: {
      ...
      // 优先从环境变量读取，然后从数据库读取，最后从配置文件读取
      defaultTeamId:
        process.env.DEFAULT_TEAM_ID ||
        defaultTeamMemberInfo.defaultTeamId ||
        fileRes.systemEnv?.defaultTeamId ||
        '',
      defaultTmbId:
        process.env.DEFAULT_TMB_ID ||
        defaultTeamMemberInfo.defaultTmbId ||
        fileRes.systemEnv?.defaultTmbId ||
        '',
      defaultUserId:
        process.env.DEFAULT_USER_ID ||
        defaultTeamMemberInfo.defaultUserId ||
        fileRes.systemEnv?.defaultUserId ||
        ''
    },
