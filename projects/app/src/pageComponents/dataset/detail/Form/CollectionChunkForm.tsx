...
const CollectionChunkForm = ({ form }: { form: UseFormReturn<CollectionChunkFormType> }) => {
  ...
  const autoIndexesModel = watch('autoIndexesModel');
  const autoIndexesSize = watch('autoIndexesSize');
  ...
  // Auto indexes model list
  const autoIndexesModelList = useMemo(() => {
    return llmModelList.map((item) => ({
      label: item.name,
      value: item.model
    }));
  }, [llmModelList]);
  ...
  return (
    <>
      ...
      <Box mt={6}>
        ...
        {/* Auto indexes configuration */}
        {autoIndexes && trainingType === DatasetCollectionDataProcessModeEnum.chunk && (
          <Box mt={4} p={4} bg={'myGray.50'} borderRadius={'md'}>
            <Grid gridTemplateColumns={'1fr 1fr'} rowGap={[2, 4]} columnGap={[3, 7]}>
              <Box>
                ...
                <MySelect
                  list={autoIndexesModelList}
                  value={autoIndexesModel}
                  onChange={(val: string) => setValue('autoIndexesModel', val)}
                  placeholder="选择用于生成问题的语言模型"
                />
              </Box>
              <Box>
                <FormLabel fontSize={'sm'} mb={2}>
