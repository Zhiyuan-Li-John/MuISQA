...
const InputDataModal = ({
  ...
  useEffect(() => {
    ...
    // Wait for collection data to be loaded
    if (!collection || collection === defaultCollectionDetail) return;

    // For new data (dataId is empty string), set default tab based on collection type
    if (dataId === '') {
    ...
    // For existing data, set tab based on data content
    if (dataItem) {
      setCurrentTab(dataItem.a ? TabEnum.qa : TabEnum.chunk);
    }
  }, [collection, dataItem, currentTab, dataId]);
