...
import EnhanceDatasetIndexModal from './EnhanceDatasetIndexModal';
...
const Header = ({ hasTrainingData }: { hasTrainingData: boolean }) => {
  ...
  // 数据集索引增强模态框状态
  const [enhanceDatasetIndexModal, setEnhanceDatasetIndexModal] = React.useState(false);
  ...
  return (
    <MyBox display={['block', 'flex']} alignItems={'center'} gap={2}>
      ...
      {datasetDetail.permission.hasWritePer && (
        <Flex mt={[3, 0]} gap={2} flexWrap="wrap">
          ...
          {/* 数据集索引增强按钮 */}
          {datasetDetail?.type === DatasetTypeEnum.dataset && total > 0 && (
            <Button
              variant="whitePrimary"
              size={['sm', 'md']}
              leftIcon={<MyIcon name={'core/dataset/indexEnhance'} w={'16px'} />}
              onClick={() => setEnhanceDatasetIndexModal(true)}
            >
      ...
      {/* 数据集索引增强模态框 */}
      {enhanceDatasetIndexModal && (
        <EnhanceDatasetIndexModal
          isOpen={true}
          onClose={() => setEnhanceDatasetIndexModal(false)}
          datasetId={datasetDetail._id}
