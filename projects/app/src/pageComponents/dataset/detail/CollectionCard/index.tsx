...
import EnhanceIndexModal from './EnhanceIndexModal';
...
const CollectionCard = () => {
  ...
  const [enhanceIndexCollection, setEnhanceIndexCollection] = useState<{
    collectionId: string;
  ...
  return (
    <MyBox isLoading={isLoading} h={'100%'} py={[2, 4]}>
      <Flex ref={BoxRef} flexDirection={'column'} py={[1, 0]} h={'100%'} px={[2, 6]}>
                        ...
                        menuList={[
                          {
                            children: [
                              ...(collectionCanSync(collection.type)
                                ...
                                : []),
                              // 索引增强选项 - 只对非文件夹类型的集合显示
                              ...(collection.type !== DatasetCollectionTypeEnum.folder &&
                              collection.dataAmount > 0
                                ? [
                                    {
                                      label: (
                                        <Flex alignItems={'center'}>
                                          <MyIcon
                                            name={'core/dataset/indexEnhance'}
                                            w={'0.9rem'}
                                            mr={2}
                                          />
                                          {t('dataset:enhance_index')}
                                        </Flex>
                                      ),
                                      onClick: () =>
                                        setEnhanceIndexCollection({
                                          collectionId: collection._id,
                                          collectionName: collection.name,
                                          dataCount: collection.dataAmount
                                        })
                                    }
                                  ]
                                : []),
        ...
        {!!enhanceIndexCollection && (
          <EnhanceIndexModal
            isOpen={true}
            onClose={() => setEnhanceIndexCollection(undefined)}
            collectionId={enhanceIndexCollection.collectionId}
            collectionName={enhanceIndexCollection.collectionName}
            dataCount={enhanceIndexCollection.dataCount}
            onSuccess={() => {
