...
export class MilvusCtrl {
  ...
  embRecall = async (props: EmbeddingRecallCtrlProps): Promise<EmbeddingRecallResponse> => {
    ...
    const filter = `(teamId == "${teamId}") and (datasetId in [${datasetIds.map((id) => `"${id}"`).join(',')}]) ${collectionIdQuery} ${forbidColQuery}`;
  ...
  /**
   * 更新 teamId - Milvus 不支持直接更新，需要先查询、删除、再插入
   */
  updateTeamId = async ({
    oldTeamId,
    newTeamId,
    ...
    try {
      ...
      const queryResult = await client.query({
        ...
        filter: `(teamId == "${String(oldTeamId)}") and (datasetId in [${datasetIds.map(id => `"${String(id)}"`).join(',')}])`
      });

      const vectorsToUpdate = queryResult.data as {
        id: string;
        vector: number[];
        teamId: string;
        datasetId: string;
      ...
      // 2. 删除旧数据
      const idsToDelete = vectorsToUpdate.map(item => String(item.id));
      await client.delete({
        collection_name: DatasetVectorTableName,
      ...
      // 3. 插入更新后的数据
      const newData = vectorsToUpdate.map(item => ({
        id: Number(item.id),
        vector: item.vector,
      ...
      return { updatedCount: vectorsToUpdate.length };
    } catch (error) {
      if (retry <= 0) {
        return Promise.reject(error);
      }
      await delay(500);
      return this.updateTeamId({
        oldTeamId,
        newTeamId,
        datasetIds,
        retry: retry - 1
