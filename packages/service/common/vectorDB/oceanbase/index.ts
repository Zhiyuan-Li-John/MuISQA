...
export class ObVectorCtrl {
  ...
  updateTeamId = async ({
    ...
    try {
      const result = await ObClient.update(DatasetVectorTableName, {
        values: [{ key: 'team_id', value: String(newTeamId) }],
        where: [
          ['team_id', String(oldTeamId)],
          'and',
          ['dataset_id', `IN (${datasetIds.map((id) => `'${String(id)}'`).join(',')})`]
        ]
      });

      // OceanBase 返回的是 mysql2 的结果，需要正确处理
