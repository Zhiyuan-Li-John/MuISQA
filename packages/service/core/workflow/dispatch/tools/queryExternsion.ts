...
export const dispatchQueryExtension = async ({
  ...
  // 从variables.searchParams中获取预定义的扩展问题列表和HYFE设置
  const predefinedQueries = variables?.searchParams?.datasetSearchExtensionQueries;
  const useHyfe = variables?.searchParams?.datasetSearchUsingHyfe;
