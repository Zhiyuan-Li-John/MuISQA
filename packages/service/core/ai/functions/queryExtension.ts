...
import { hyfeQueryDecomposition } from './hyfeQueryDecomposition';
...
export const queryExtension = async ({
  ...
  // 如果提供了预定义的问题列表，直接返回，跳过LLM调用
  if (predefinedQueries && predefinedQueries.length > 0) {
    return {
      rawQuery: query,
      extensionQueries: predefinedQueries,
      model,
      inputTokens: 0,
      outputTokens: 0
    };
  ...
  // 如果使用HYFE问题分解
  if (useHyfe) {
    const { decomposedQueries, inputTokens, outputTokens } = await hyfeQueryDecomposition({
      query,
      model
    });
    ...
    return {
      rawQuery: query,
      extensionQueries: decomposedQueries,
      model,
      inputTokens,
      outputTokens
    };
