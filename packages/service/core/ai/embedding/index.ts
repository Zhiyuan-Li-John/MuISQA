...
// 判断当前是否使用PgVector数据库
function isPgVector(): boolean {
  const PG_ADDRESS = process.env.PG_URL;
  const OCEANBASE_ADDRESS = process.env.OCEANBASE_URL;
  const MILVUS_ADDRESS = process.env.MILVUS_ADDRESS;

  // 根据环境变量判断，优先级：PG > OceanBase > Milvus，默认为PG
  if (PG_ADDRESS) return true;
  if (OCEANBASE_ADDRESS) return false;
  if (MILVUS_ADDRESS) return false;

  return true; // 默认使用PgVector
}
      ...
      .then(async (res) => {
        ...
        const [tokens, vectors] = await Promise.all([
          ...
          Promise.all(
            res.data
              .map((item) => {
                // 只有在使用PgVector时才执行unityDimensional处理
                if (isPgVector()) {
                  return unityDimensional(item.embedding);
                }
