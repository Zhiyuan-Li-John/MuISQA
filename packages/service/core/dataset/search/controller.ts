...
export async function searchDatasetData(
  ...
  */
  const filterCollectionByMetadata = async (): Promise<string[] | undefined> => {
    ...
    let specifiedCollectionIdList: string[] | undefined = undefined;
    ...
    try {
      ...
      // 处理直接指定的collectionIds
      if (jsonMatch?.collectionIds?.$in && Array.isArray(jsonMatch.collectionIds.$in)) {
        specifiedCollectionIdList = jsonMatch.collectionIds.$in.map((id: string) => String(id));
      }
      ...
      const collectionIds = (() => {
        ...
        // 如果有多个过滤条件，取交集
        return allFilters.reduce((acc, current) => {
...
export type DefaultSearchDatasetDataProps = SearchDatasetDataProps & {
  ...
  datasetSearchExtensionQueries?: string[];
  datasetSearchUsingHyfe?: boolean;
};
export const defaultSearchDatasetData = async ({
  datasetSearchUsingExtensionQuery,
  datasetSearchExtensionModel,
  datasetSearchExtensionBg,
  datasetSearchExtensionQueries,
  ...
  // 处理HYFE逻辑：当datasetSearchUsingHyfe为true且datasetSearchUsingExtensionQuery为true且datasetSearchExtensionQueries为空时
  let finalPredefinedQueries = datasetSearchExtensionQueries;
  let useHyfe = false;
  ...
  if (
    ...
    datasetSearchUsingExtensionQuery &&
    extensionModel &&
    (!datasetSearchExtensionQueries || datasetSearchExtensionQueries.length === 0)
  ) {
    useHyfe = true;
  }
  ...
  const { concatQueries, extensionQueries, rewriteQuery, aiExtensionResult } =
    await datasetSearchQueryExtension({
      ...
      histories,
      predefinedQueries: finalPredefinedQueries,
      useHyfe
    });
