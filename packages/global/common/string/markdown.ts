...
export const matchMdImg = (text: string) => {
  // 支持两种格式：data:image/png;base64,xxx 和 data:image;base64,xxx
  const base64Regex = /!\[([^\]]*)\]\((data:image(?:\/[^;]+)?;base64[^)]+)\)/g;
  const imageList: ImageType[] = [];

  text = text.replace(base64Regex, (match, altText, base64Url) => {
    const uuid = `IMAGE_${getNanoid(12)}_IMAGE`;
    const urlParts = base64Url.split(';')[0].split(':')[1];
    // 如果有具体的MIME类型（如image/png），提取它；否则默认为image
    const mime = urlParts.includes('/') ? urlParts : 'image/png';
