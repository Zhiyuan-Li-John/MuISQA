...
export type DatasetSearchParamsType = {
  ...
  limit?: number; // 引用上限 (tokens)
  searchMode?: 'embedding' | 'fullTextRecall' | 'mixedRecall'; // 搜索方式
  embeddingWeight?: number; // 向量权重 (混合检索时)
  ...
  // 问题优化参数
  datasetSearchUsingExtensionQuery?: boolean; // 是否使用问题优化
  datasetSearchExtensionModel?: string; // 问题优化模型
  datasetSearchExtensionBg?: string; // 问题优化背景
  datasetSearchExtensionQueries?: string[]; // 预设的扩展问题列表，如果提供则跳过LLM生成
  datasetSearchUsingHyfe?: boolean; // 是否使用HYFE问题分解

  // 深度搜索参数
  datasetDeepSearch?: boolean; // 是否使用深度搜索
  datasetDeepSearchModel?: string; // 深度搜索模型
  datasetDeepSearchMaxTimes?: number; // 深度搜索最大次数
  datasetDeepSearchBg?: string; // 深度搜索背景
};

export type DatasetVariablesType = {
  // 数据集配置
  datasets?: Array<{
    datasetId: string;
  ...
  // 搜索参数配置
  searchParams?: DatasetSearchParamsType;
};
